			...
			switch (res.statusCode) {
				case 200: // success
					...
					if (file) {
						...
						var buf = '';
						if (res.headers['content-encoding'] == 'gzip') {
							var enc = res.headers['content-type'].split(';')[1].trim().split('=')[1].trim().toLowerCase();
							var gunzip = gunzipstream.wrap(res, {encoding: enc});
							gunzip.on('error', function (err) {
								if ( ! aborted) {
									aborted = true;
									req.abort();
							...
							var enc = res.headers['content-type'].split(';')[1].trim().split('=')[1].trim().toLowerCase();
							if(enc != 'utf-8') res.setEncoding('binary'); // handling of iso-8859-x (and maybe other) encodings
							res.on('data', function (data) {
								if ( ! aborted) {
									buf += data;
									if (options.maxbody) {
